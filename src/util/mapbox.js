...
export class RequestManager {
    ...
    normalizeTileURL(tileURL: string, use2x?: boolean, rasterTileSize?: number): string {
        ...
        if (tileURL && !isMapboxURL(tileURL)) {
            const urlObject2 = parseUrl(tileURL);
            const accessToken = this._customAccessToken || getAccessToken(urlObject.params) || config.ACCESS_TOKEN;
            if (config.REQUIRE_ACCESS_TOKEN && accessToken) {
                urlObject2.params.push(`platform=web`);
                urlObject2.params.push(`sku=${this._skuToken}`);
                urlObject2.params.push(`appSecret=${accessToken || ''}`);
            }
            return formatUrl(urlObject2);
        }
    ...
    _makeAPIURL(urlObject: UrlObject, accessToken: string | null | void): string {
        ...
        urlObject.params.push(`appSecret=${accessToken || ''}`);
        return formatUrl(urlObject);
    }
}
